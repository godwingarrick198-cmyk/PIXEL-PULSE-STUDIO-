    ...
    async def discover_prospects(self, query):
        ...
        try:
            async with httpx.AsyncClient(timeout=25) as c:
                r=await c.post(self.endpoint,json={'query':self.query,'variables':{'first':30}},headers={'Authorization':f'Bearer {s.PRODUCT_HUNT_ACCESS_TOKEN}'})
                r.raise_for_status(); edges=r.json().get('data',{}).get('posts',{}).get('edges',[])
