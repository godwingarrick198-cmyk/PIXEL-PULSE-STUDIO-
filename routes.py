...
def create_campaign(payload:CampaignCreate,db:Session=Depends(get_db)):
    c=campaigns.create(db,payload.name,payload.target_prospects,payload.duration_days,payload.industries,payload.countries,payload.services,payload.priority); return safe_campaign(c)
...
def resume(id:str,db:Session=Depends(get_db)): return change(id,'RUNNING',db)
...
async def create_order(payload:OrderCreate,db:Session=Depends(get_db)):
    ...
    if package not in prices: raise HTTPException(400,'Unsupported package')
    cust=Customer(name=payload.customer_name,company_name=payload.company_name,email=payload.email,prospect_id=payload.prospect_id); db.add(cust); db.flush()
    o=Order(order_id='PPS-ORD-'+uuid.uuid4().hex[:12].upper(),customer_id=cust.id,prospect_id=payload.prospect_id,package=package,amount=prices[package],currency=s.SERVICE_CURRENCY,status='PENDING'); db.add(o); db.flush()
    ref='PPS-'+o.order_id+'-'+uuid.uuid4().hex[:6].upper(); pay=Payment(payment_id='PAY-'+uuid.uuid4().hex[:10],order_id=o.id,reference=ref,amount=o.amount,currency=o.currency); db.add(pay); db.commit()
    try:
...
async def flutterwave_webhook(request:Request,db:Session=Depends(get_db),flutterwave_signature:str|None=Header(default=None,alias='flutterwave-signature'),verif_hash:str|None=Header(default=None,alias='verif-hash')):
    ...
    if not flw.sign_valid(raw,sig) and not (s.TEST_MODE and sig=='TEST'): raise HTTPException(401,'Invalid webhook signature')
    payload=json.loads(raw or b'{}'); event_id=str(payload.get('id') or payload.get('webhook_id') or hashlib.sha256(raw).hexdigest())
    if db.scalar(select(PaymentEvent).where(PaymentEvent.event_id==event_id)): return {'status':'ok','duplicate':True}
    ev=PaymentEvent(event_id=event_id,payload=payload); db.add(ev); db.commit()
    data=payload.get('data',{}); txid=data.get('id'); ref=data.get('tx_ref') or data.get('txRef')
    pay=db.scalar(select(Payment).where(Payment.reference==ref)) if ref else None
    if pay and txid:
        ok,verified=await flw.verify(txid,pay.reference,pay.amount,pay.currency)
        if ok:
            pay.status='PAID'; pay.provider_transaction_id=str(txid); o=db.get(Order,pay.order_id); o.status='PAID'; form=db.scalar(select(OnboardingForm).where(OnboardingForm.order_id==o.id)); form=form or OnboardingForm(order_id=o.id,data={}); db.add(form); ev.processed=True; db.commit(); events.event('PAYMENT_VERIFIED',order_id=o.order_id)
    return {'status':'ok'}
...
def update_onboarding(id:int,payload:OnboardingUpdate,db:Session=Depends(get_db)):
    o=db.get(Order,id)
    if not o: raise HTTPException(404,'Order not found')
    form=db.scalar(select(OnboardingForm).where(OnboardingForm.order_id==id))
    if not form: form=OnboardingForm(order_id=id,data={}); db.add(form)
    form.data={**(form.data or {}),**payload.data}; form.status='COMPLETE' if all(form.data.get(k) not in (None,'',[]) for k in ['customer_name','company_name','email','presentation_type','purpose','audience','number_of_slides','style']) else 'INCOMPLETE'; db.commit()
...
async def upload_order_file(id:int,file:UploadFile=File(...),db:Session=Depends(get_db)):
    o=db.get(Order,id)
    if not o: raise HTTPException(404,'Order not found')
    allowed={'.pptx','.ppt','.pdf','.docx','.doc','.txt','.csv','.png','.jpg','.jpeg','.svg'}; ext=os.path.splitext(file.filename or '')[1].lower()
    ...
    sha=hashlib.sha256(data).hexdigest(); base=os.path.join('storage','uploads',str(id)); os.makedirs(base,exist_ok=True); safe=os.path.basename(file.filename or 'upload'); path=os.path.join(base,sha+'_'+safe)
    with open(path,'wb') as f:f.write(data)
    uf=UploadedFile(order_id=id,filename=safe,stored_path=path,mime_type=file.content_type or 'application/octet-stream',size_bytes=len(data),sha256=sha);db.add(uf);db.commit();return {'id':uf.id,'filename':safe,'size_bytes':len(data)}
