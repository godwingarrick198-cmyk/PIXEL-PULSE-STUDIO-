...
async def flutterwave_webhook(request:Request,db:Session=Depends(get_db),flutterwave_signature:str|None=Header(default=None,alias='flutterwave-signature'),verif_hash:str|None=Header(default=None,alias='verif-hash')):
    ...
    ev=PaymentEvent(event_id=event_id,payload=payload); db.add(ev); db.commit(); data=payload.get('data',{}); txid=data.get('id'); ref=data.get('tx_ref') or data.get('txRef'); pay=db.scalar(select(Payment).where(Payment.reference==ref)) if ref else None
    if pay and txid:
