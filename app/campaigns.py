...
class CampaignService:
    ...
    def create(self, db, name, target, duration_days=1, industries=None, countries=None, services=None, priority="NORMAL"):
        ...
        c = Campaign(
            campaign_id="PPS-" + uuid.uuid4().hex[:10].upper(),
            name=name,
            status="DRAFT",
            priority=priority,
            target_prospects=target,
            completed_prospects=0,
            remaining_prospects=target,
            start_time=start,
            end_time=end,
            daily_limit=daily,
            ...
            outreach_limit=self.s.MAX_DAILY_OUTREACH,
        )
        db.add(c)
        db.commit()
        db.refresh(c)
    ...
    def due_campaigns(self, db):
        return db.scalars(
            ...
            .order_by(Campaign.priority.desc(), Campaign.created_at)
        ).all()
