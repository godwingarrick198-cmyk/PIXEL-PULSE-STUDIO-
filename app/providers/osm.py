...
class OSMProvider(ProspectProvider):
    ...
    async def discover_prospects(self, query):
        ...
        tags={'real estate':['office=estate_agent','shop=estate_agent'],'consultant':['office=consulting'],'agency':['office=advertising_agency'],'education':['amenity=college','amenity=school'],'business':['office=*']}
        selected=tags.get(str(query.get('industry','business')).lower(),tags['business']); parts=[]
        for tag in selected:
