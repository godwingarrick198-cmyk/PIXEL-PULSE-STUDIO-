    ...
    async def discover_prospects(self, query):
        ...
        try:
            ...
            for a in soup.select('a.result__a')[:30]:
                ...
                out.append({'company_name':re.sub(r'\s+',' ',title)[:255],'website':f'{p.scheme}://{p.netloc}','domain':domain,'country':query.get('country'),'industry':query.get('industry'),'description':title,'source':'web','source_id':domain,'source_url':href})
            return out
