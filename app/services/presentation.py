...
from app.models.entities import Project, Slide, OnboardingForm, UploadedFile
...
class PresentationService:
    ...
    def generate(self, db, order_id):
        ...
        strategy=self._strategy(form.data or {}, self._source_text(db,order_id))
        ...
        for idx,s in enumerate(strategy['slides'],1):
            ...
            title=slide.shapes.add_textbox(Inches(.7),Inches(.55),Inches(11.9),Inches(1.0)).text_frame
            title.text=s['title']; title.paragraphs[0].font.size=Pt(30); title.paragraphs[0].font.bold=True
            body=slide.shapes.add_textbox(Inches(.9),Inches(1.75),Inches(11.4),Inches(4.9)).text_frame
            body.clear()
            bullets=s['bullets'] or ['Placeholder — add verified source information here.']
        ...
        for idx,s in enumerate(strategy['slides'],1):
            c.setFont('Helvetica-Bold',26); c.drawString(.7*inch,page_h-1*inch,s['title'][:70])
            c.setFont('Helvetica',16); y=page_h-1.7*inch
            for b in (s['bullets'] or ['Placeholder — add verified source information here.']):
                c.drawString(.9*inch,y,('• '+b)[:105]); y-=.35*inch
                if y<.7*inch: break
            c.setFont('Helvetica',9); c.drawRightString(page_w-.5*inch,page_h-.4*inch,f'{idx}/{len(strategy["slides"])}')
            c.showPage()
        c.save()
        project.pptx_path=pptx_path; project.pdf_path=pdf_path; project.status='READY'; project.quality_score=100; project.updated_at=datetime.utcnow()
