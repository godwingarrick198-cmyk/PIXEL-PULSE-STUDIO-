...
class CampaignService:
    ...
    def create(self,db,name,target,duration_days=1,industries=None,countries=None,services=None,priority='NORMAL'):
        start=datetime.now(timezone.utc); end=start+timedelta(days=duration_days); daily=min(math.ceil(target/duration_days),self.s.MAX_DAILY_PROSPECTS)
        c=Campaign(campaign_id='PPS-'+uuid.uuid4().hex[:10].upper(),name=name,status='DRAFT',target_prospects=target,remaining_prospects=target,start_time=start,end_time=end,daily_limit=daily,industries=industries or [],countries=countries or [],services=services or [],outreach_limit=self.s.MAX_DAILY_OUTREACH)
