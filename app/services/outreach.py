...
class OutreachService:
    ...
    async def send_email(self, to_email, subject, body):
        ...
        await aiosmtplib.send(msg, hostname=self.s.SMTP_HOST, port=self.s.SMTP_PORT, start_tls=True, username=self.s.SMTP_USERNAME, password=self.s.SMTP_PASSWORD)
    ...
    def suppressed(self, db, prospect):
        ...
        q = select(SuppressionList.id).where(
            (SuppressionList.email == email) if email else False
        )
        if domain:
            q = q.where((SuppressionList.email == email) | (SuppressionList.domain == domain)) if email else q.where(SuppressionList.domain == domain)
    ...
    async def send_one(self, db, campaign_code, prospect_id):
        ...
        row = OutreachMessage(message_uid=uid, prospect_id=prospect.id, campaign_id=campaign.id, channel='email', direction='outbound', subject=subject, body=body, status='QUEUED')
        db.add(row); db.commit(); db.refresh(row)
        try:
            await self.send_email(prospect.contact_email, subject, body)
            row.status='SENT'; row.sent_at=datetime.now(timezone.utc); prospect.last_contacted_at=row.sent_at
            cp = db.scalar(select(CampaignProspect).where(CampaignProspect.campaign_id==campaign.id, CampaignProspect.prospect_id==prospect.id))
            if cp: cp.status='CONTACTED'
            db.commit()
            ...
            row.status='FAILED'; db.commit(); events.event('ERROR',component='outreach_send',error=str(e),prospect_id=prospect.id)
