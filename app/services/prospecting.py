...
from app.models.entities import Prospect, SuppressionList, CampaignProspect
...
class ProspectingService:
    ...
    def suppressed(self,db,p):
        dom=self.normalize_domain(p.get('website') or p.get('domain')); email=(p.get('contact_email') or '').lower()
        return db.scalar(select(SuppressionList.id).where(or_(SuppressionList.email==email if email else False, SuppressionList.domain==dom if dom else False, SuppressionList.company.ilike(p.get('company_name',''))))) is not None
    async def discover(self,db,query,limit):
        ...
        for raw in all_items:
            ...
            seen.add(key)
            existing=db.scalar(select(Prospect).where(or_(Prospect.domain==domain if domain else False, Prospect.contact_email==email if email else False, func.lower(Prospect.company_name)==name.lower())))
