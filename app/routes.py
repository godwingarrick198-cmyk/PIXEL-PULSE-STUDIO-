...
def safe_campaign(c):
    return {
        ...
        "target_prospects": c.target_prospects, "completed_prospects": c.completed_prospects,
        "remaining_prospects": c.remaining_prospects, "daily_limit": c.daily_limit,
        "priority": c.priority, "industries": c.industries, "countries": c.countries,
        "services": c.services, "start_time": c.start_time, "end_time": c.end_time,
    }
...
def status(db: Session = Depends(get_db)):
    ...
    return {
        ...
        "active_orders": db.scalar(
            select(func.count(Order.id)).where(Order.status.in_(["PAID", "IN_PRODUCTION", "QC", "READY"]))
        ) or 0,
    }
...
def list_campaigns(db: Session = Depends(get_db)):
    return [safe_campaign(c) for c in db.scalars(select(Campaign).order_by(Campaign.created_at.desc())).all()]


@router.post("/campaigns")
def create_campaign(payload: CampaignCreate, db: Session = Depends(get_db)):
    c = campaigns.create(db, payload.name, payload.target_prospects, payload.duration_days,
                          payload.industries, payload.countries, payload.services, payload.priority)
...
def prospects(limit: int = 50, db: Session = Depends(get_db)):
    return [
        {"id": p.id, "company_name": p.company_name, "website": p.website,
         "industry": p.industry, "service_match": p.service_match,
         "score": p.qualification_score, "status": p.status, "email": p.contact_email}
        for p in db.scalars(select(Prospect).order_by(Prospect.created_at.desc()).limit(min(limit, 200))).all()
    ]
...
def prospect(id: int, db: Session = Depends(get_db)):
    ...
    return {"id": p.id, "company_name": p.company_name, "website": p.website,
            "industry": p.industry, "description": p.description, "country": p.country,
            "service_match": p.service_match, "qualification": p.qualification_json}
...
def orders(db: Session = Depends(get_db)):
    return [
        {"id": o.id, "order_id": o.order_id, "package": o.package, "amount": o.amount,
         "currency": o.currency, "status": o.status, "created_at": o.created_at}
        for o in db.scalars(select(Order).order_by(Order.created_at.desc())).all()
    ]
...
def order(id: int, db: Session = Depends(get_db)):
    ...
    return {"id": o.id, "order_id": o.order_id, "package": o.package,
...
async def create_order(payload: OrderCreate, db: Session = Depends(get_db)):
    ...
    cust = Customer(name=payload.customer_name, company_name=payload.company_name,
                    email=payload.email, prospect_id=payload.prospect_id)
    db.add(cust)
    db.flush()

    o = Order(order_id="PPS-ORD-" + uuid.uuid4().hex[:12].upper(), customer_id=cust.id,
              prospect_id=payload.prospect_id, package=package, amount=prices[package],
              currency=s.SERVICE_CURRENCY, status="PENDING")
    db.add(o)
    db.flush()

    ref = "PPS-" + o.order_id + "-" + uuid.uuid4().hex[:6].upper()
    pay = Payment(payment_id="PAY-" + uuid.uuid4().hex[:10], order_id=o.id, reference=ref,
                  amount=o.amount, currency=o.currency)
    db.add(pay)
    db.commit()

    try:
        result = await flw.create_payment(ref, o.amount, o.currency, payload.email, payload.customer_name, o.order_id)
        pay.payment_url = result.get("link")
        db.commit()
        events.event("PAYMENT_CREATED", order_id=o.order_id)
    except Exception as e:
        db.rollback()
        raise HTTPException(502, f"Payment creation failed: {e}")

    return {"order_id": o.order_id, "reference": ref, "payment_url": pay.payment_url,
            "amount": o.amount, "currency": o.currency}
...
async def flutterwave_webhook(
    ...
    if not flw.sign_valid(raw, sig) and not (s.TEST_MODE and sig == "TEST"):
        raise HTTPException(401, "Invalid webhook signature")

    payload = json.loads(raw or b"{}")
    event_id = str(payload.get("id") or payload.get("webhook_id") or hashlib.sha256(raw).hexdigest())
    if db.scalar(select(PaymentEvent).where(PaymentEvent.event_id == event_id)):
        return {"status": "ok", "duplicate": True}

    ev = PaymentEvent(event_id=event_id, payload=payload)
    db.add(ev)
    db.commit()
    ...
    if pay and txid:
        ok, _ = await flw.verify(txid, pay.reference, pay.amount, pay.currency)
        if ok:
            pay.status = "PAID"
            ...
            if not form:
                form = OnboardingForm(order_id=o.id, data={})
                db.add(form)
            ev.processed = True
            db.commit()
            events.event("PAYMENT_VERIFIED", order_id=o.order_id)
...
def update_onboarding(id: int, payload: OnboardingUpdate, db: Session = Depends(get_db)):
    ...
    if not form:
        form = OnboardingForm(order_id=id, data={})
        db.add(form)
    form.data = {**(form.data or {}), **payload.data}
    required = ["customer_name", "company_name", "email", "presentation_type",
                "purpose", "audience", "number_of_slides", "style"]
    form.status = "COMPLETE" if all(form.data.get(k) not in (None, "", []) for k in required) else "INCOMPLETE"
    db.commit()
...
async def upload_order_file(id: int, file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    allowed = {".pptx", ".ppt", ".pdf", ".docx", ".doc", ".txt", ".csv", ".png", ".jpg", ".jpeg", ".svg"}
    ...
    base = os.path.join("storage", "uploads", str(id))
    os.makedirs(base, exist_ok=True)
    safe = os.path.basename(file.filename or "upload")
    path = os.path.join(base, sha + "_" + safe)

    with open(path, "wb") as f:
        f.write(data)

    uf = UploadedFile(order_id=id, filename=safe, stored_path=path,
                      mime_type=file.content_type or "application/octet-stream",
                      size_bytes=len(data), sha256=sha)
    db.add(uf)
    db.commit()
    return {"id": uf.id, "filename": safe, "size_bytes": len(data)}
