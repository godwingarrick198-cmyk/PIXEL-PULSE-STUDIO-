...
SERVICES = {
    'startup':'Investor Pitch Deck','saas':'Investor/Sales Deck','technology company':'Investor/Sales Deck',
    'marketing agency':'Client Sales Deck','agency':'Client Sales Deck','consultant':'Proposal Deck','consulting':'Proposal Deck',
    'real estate':'Property/Investment Presentation','property':'Property/Investment Presentation','corporate':'Corporate Presentation',
    'business':'Corporate Presentation','coach':'Webinar/Training Presentation','education':'Training Presentation',
    'e-commerce':'Product Presentation','product':'Product Presentation','webinar':'Webinar Presentation','training':'Training Presentation'
}
...
class AIService:
    ...
    def _fallback(self, prospect):
        ...
        for key,val in SERVICES.items():
            if key in text: service=val; break
    ...
    def generate_outreach(self, prospect, service):
        ...
        body=(f'Hi {name},\n\nPixel Pulse Studio creates professional {service.lower()}s for businesses that need clear, polished presentations. '
    ...
    def sales_reply(self, conversation, customer_question):
        ...
        try:
            ...
            events.event('ERROR',component='gemini_sales',error=str(e)); return 'Thanks for your message. I need the owner to review that request before I confirm it.'
    ...
    def presentation_strategy(self, onboarding, source_text):
        ...
        try:
            r=self.client.models.generate_content(model='gemini-2.5-flash',contents=prompt,config={'response_mime_type':'application/json'})
            return json.loads(r.text)
        except Exception as e:
            events.event('ERROR',component='gemini_strategy',error=str(e)); return {'style':onboarding.get('style','Premium Minimal'),'slides':[{'layout':a,'title':b,'bullets':[]} for a,b in default]}
